...
/**
 * Calculates the histogram of every color channel of the image. does not alter the input image.
 * @author Luca Rossetto
 *
 */
public class HistogramCalculator extends AbstractImageManipulator implements IntensityMap {
	
	public static final int RETURN_RED = 0, RETURN_GREEN = 1, RETURN_BLUE = 2;

	private int[] hist_red = new int[256], hist_green = new int[256], hist_blue = new int[256];
	...
	public void manipulate(JMTFImage input) {
		...
		for(int i = 0; i < pixels.length; ++i){
			this.hist_red[JMTFImage.getRed(pixels[i])]++;
			this.hist_green[JMTFImage.getGreen(pixels[i])]++;
			this.hist_blue[JMTFImage.getBlue(pixels[i])]++;
		}
